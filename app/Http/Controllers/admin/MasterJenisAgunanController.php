...
use App\Models\MasterJenisAgunanModel;
use Illuminate\Http\Request;
use Carbon\Carbon;
use Illuminate\Support\Facades\Redirect;
use Illuminate\Support\Facades\Validator;

class MasterJenisAgunanController extends Controller
{
     function index(Request $r)
    {
        ...
        $data['data'] = MasterJenisAgunanModel::get();
        $data['tag'] = [];
        $data['kategori'] = [];
        ...
        return view('admin.masterpengajuan.jenis_agunan', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $i = MasterJenisAgunanModel::updateOrCreate(['id' => $request->txtId], $input);
        if ($i) {
      ...
      public function destroy($id)
        {
            $d = MasterJenisAgunanModel::find($id);
            if ($d) {
                $d->delete();
