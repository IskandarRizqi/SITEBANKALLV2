@extends('frontend.bprman.layout.main')

@section('content')
<<<<<<< Updated upstream
<div class="breadcumb-area style2 bg-smoke4">
    <div class="breadcumb-wrapper" data-bg-src="frontend/bprbahari/assets/img/bg/breadcumb-bg.jpg">
        <div class="container">
            <div class="breadcumb-content">
                <h1 class="breadcumb-title">Privacy Policy</h1>
                <ul class="breadcumb-menu">
                    <li><a href="/">Profil</a></li>
                    <li>Privacy Policy</li>
                </ul>
            </div>
        </div>
    </div>
</div>

<!--=====SERVICE DETAILS AREA START=======-->

<div class="service-details-area-all sp">
    <div class="container">
        <div class="row justify-content-center">
            <!-- Menggunakan col-lg-10 untuk memberi margin di samping -->
            <div class="col-lg-11 m-auto">
                <div class="service-details-post">
                    <article>
                        <div class="details-post-area">
                            <!-- Gambar di tengah -->
                            <div class="image text-center">
                                <img src="frontend/bprjas/assets/img/profil/police.jpg" alt="Privacy Policy">
=======
    <!--=====SERVICE DETAILS AREA START=======-->

    <div class="service-details-area-all sp">
        <div class="container">
            <div class="row justify-content-center">
                <!-- Menggunakan col-lg-10 untuk memberi margin di samping -->
                <div class="col-lg-11 m-auto">
                    <div class="service-details-post">
                        <article>
                            <div class="details-post-area" style="padding-top: 12%;">
                                <!-- Gambar di tengah -->
                                <div class="text-center">
                                    <h2 class="text-center">Kebijakan Privasi</h2>
                                </div>
                                <div class="space30"></div>
                                <div class="heading1">
                                  <br>
                                    <div class="space16"></div>
                                    <p style="text-align: justify;">
                                        PT. BPR Multi Arthanusa berkomitmen untuk menjaga privasi dan keamanan data nasabah
                                        serta pengunjung situs web kami. Dengan mengakses situs ini, Anda menyetujui
                                        praktik yang dijelaskan dalam kebijakan ini.
                                    </p>
                                    <p class="mb-0"><strong>1. Informasi yang Dikumpulkan</strong></p>
                                    <p class="mb-0" style="text-align: justify;">
                                        Kami dapat mengumpulkan informasi pribadi, seperti:
                                    </p>
                                    <ul style="text-align: justify; padding-left: 20px; margin-bottom: 15px;">
                                        <li>Nama, alamat, nomor telepon, email.</li>
                                        <li>Data transaksi atau keuangan jika Anda menggunakan fitur tertentu.</li>
                                    </ul>

                                    <p class="mb-0"><strong>2. Penggunaan Informasi</strong></p>
                                    <p class="mb-0" style="text-align: justify">
                                        Informasi yang dikumpulkan digunakan untuk:
                                    </p>
                                    <ul style="text-align: justify; padding-left: 20px; margin-bottom: 15px;">
                                        <li>Memberikan layanan yang lebih baik.</li>
                                        <li>Menghubungi Anda jika diperlukan.</li>
                                        <li>Meningkatkan sistem keamanan dan kenyamanan pengguna.</li>
                                    </ul>

                                    <p class="mb-0"><strong>3. Perlindungan Data</strong></p>
                                    <p style="text-align: justify">
                                        Kami menerapkan standar keamanan yang tinggi untuk menjaga data Anda dari akses
                                        tidak sah, perubahan, pengungkapan, atau penghancuran.
                                    </p>

                                    <p class="mb-0"><strong>4. Cookies</strong></p>
                                    <p style="text-align: justify">
                                        Situs kami dapat menggunakan cookies untuk meningkatkan pengalaman pengguna.
                                        Anda bisa menonaktifkan cookies melalui pengaturan browser Anda.
                                    </p>

                                    <p class="mb-0"><strong>5. Tautan ke Situs Lain</strong></p>
                                    <p style="text-align: justify">
                                        Situs kami mungkin memuat tautan ke situs pihak ketiga. Kami tidak bertanggung
                                        jawab atas kebijakan privasi atau konten situs tersebut.
                                    </p>

                                    <p class="mb-0"><strong>6. Persetujuan</strong></p>
                                    <p style="text-align: justify">
                                        Dengan menggunakan situs ini, Anda menyetujui pengumpulan dan penggunaan data
                                        Anda sebagaimana dijelaskan dalam kebijakan ini.
                                    </p>
                                    <div class="space30"></div>

                                    <!-- Judul kedua di tengah -->
                                    <h4 class="text-center mt-5">Kebijakan Penggunaan Situs Web</h4>
                                    <div class="space16"></div>

                                    <p class="mb-0 mt-5"><strong>1. Hak Cipta & Merek Dagang</strong></p>
                                    <p style="text-align: justify">
                                        Seluruh konten di situs ini, termasuk teks, gambar, logo, dan desain, dilindungi
                                        oleh hukum dan merupakan milik PT. BPRS Taja, kecuali dinyatakan lain.
                                    </p>

                                    <p class="mb-0"><strong>2. Batasan Penggunaan</strong></p>
                                    <p class="mb-0" style="text-align: justify">
                                        Pengunjung tidak diperbolehkan:
                                    </p>
                                    <ul style="text-align: justify; padding-left: 20px; margin-bottom: 15px;">
                                        <li>Mengubah atau mendistribusikan konten tanpa izin tertulis.</li>
                                        <li>Menggunakan konten untuk tujuan komersial tanpa persetujuan.</li>
                                    </ul>

                                    <p class="mb-0"><strong>3. Ketentuan Layanan</strong></p>
                                    <p style="text-align: justify">
                                        Kami berhak mengubah informasi dan fitur di situs kapan saja. Kami tidak
                                        bertanggung jawab atas gangguan layanan karena masalah teknis.
                                    </p>

                                    <p class="mb-0"><strong>4. Keamanan Akses</strong></p>
                                    <p style="text-align: justify">
                                        Anda bertanggung jawab menjaga kerahasiaan informasi login (jika berlaku).
                                        Dilarang melakukan tindakan peretasan atau penyalahgunaan sistem.
                                    </p>

                                    <p class="mb-0"><strong>5. Persetujuan</strong></p> <!-- Mengubah nomor 6 menjadi 5 untuk konsistensi -->
                                    <p style="text-align: justify">
                                        Dengan menggunakan situs ini, Anda menyetujui pengumpulan dan penggunaan data
                                        Anda sebagaimana dijelaskan dalam kebijakan ini.
                                    </p>
                                    <div class="space30"></div>
                                </div>
>>>>>>> Stashed changes
                            </div>
                            <div class="space30"></div>
                            <div class="heading1">
                                <br>
                                <div class="space16"></div>
                                <p style="text-align: justify;">
                                    PT. BPRS Taja berkomitmen untuk menjaga privasi dan keamanan data nasabah
                                    serta pengunjung situs web kami. Dengan mengakses situs ini, Anda menyetujui
                                    praktik yang dijelaskan dalam kebijakan ini.
                                </p>
                                <p><strong>1. Informasi yang Dikumpulkan</strong></p>
                                <p style="text-align: justify;">
                                    Kami dapat mengumpulkan informasi pribadi, seperti:
                                </p>
                                <ul style="text-align: justify; padding-left: 20px;">
                                    <li>Nama, alamat, nomor telepon, email.</li>
                                    <li>Data transaksi atau keuangan jika Anda menggunakan fitur tertentu.</li>
                                </ul>

                                <p><strong>2. Penggunaan Informasi</strong></p>
                                <p style="text-align: justify">
                                    Informasi yang dikumpulkan digunakan untuk:
                                </p>
                                <ul style="text-align: justify; padding-left: 20px;">
                                    <li>Memberikan layanan yang lebih baik.</li>
                                    <li>Menghubungi Anda jika diperlukan.</li>
                                    <li>Meningkatkan sistem keamanan dan kenyamanan pengguna.</li>
                                </ul>

                                <p><strong>3. Perlindungan Data</strong></p>
                                <p style="text-align: justify">
                                    Kami menerapkan standar keamanan yang tinggi untuk menjaga data Anda dari akses
                                    tidak sah, perubahan, pengungkapan, atau penghancuran.
                                </p>

                                <p><strong>4. Cookies</strong></p>
                                <p style="text-align: justify">
                                    Situs kami dapat menggunakan cookies untuk meningkatkan pengalaman pengguna.
                                    Anda bisa menonaktifkan cookies melalui pengaturan browser Anda.
                                </p>

                                <p><strong>5. Tautan ke Situs Lain</strong></p>
                                <p style="text-align: justify">
                                    Situs kami mungkin memuat tautan ke situs pihak ketiga. Kami tidak bertanggung
                                    jawab atas kebijakan privasi atau konten situs tersebut.
                                </p>

                                <p><strong>6. Persetujuan</strong></p>
                                <p style="text-align: justify">
                                    Dengan menggunakan situs ini, Anda menyetujui pengumpulan dan penggunaan data
                                    Anda sebagaimana dijelaskan dalam kebijakan ini.
                                </p>
                                <div class="space30"></div>

                                <!-- Judul kedua di tengah -->
                                <h4 class="text-center">Kebijakan Penggunaan Situs Web</h4>
                                <div class="space16"></div>

                                <p><strong>1. Hak Cipta & Merek Dagang</strong></p>
                                <p style="text-align: justify">
                                    Seluruh konten di situs ini, termasuk teks, gambar, logo, dan desain, dilindungi
                                    oleh hukum dan merupakan milik PT. BPRS Taja, kecuali dinyatakan lain.
                                </p>

                                <p><strong>2. Batasan Penggunaan</strong></p>
                                <p style="text-align: justify">
                                    Pengunjung tidak diperbolehkan:
                                </p>
                                <ul style="text-align: justify; padding-left: 20px;">
                                    <li>Mengubah atau mendistribusikan konten tanpa izin tertulis.</li>
                                    <li>Menggunakan konten untuk tujuan komersial tanpa persetujuan.</li>
                                </ul>

                                <p><strong>3. Ketentuan Layanan</strong></p>
                                <p style="text-align: justify">
                                    Kami berhak mengubah informasi dan fitur di situs kapan saja. Kami tidak
                                    bertanggung jawab atas gangguan layanan karena masalah teknis.
                                </p>

                                <p><strong>4. Keamanan Akses</strong></p>
                                <p style="text-align: justify">
                                    Anda bertanggung jawab menjaga kerahasiaan informasi login (jika berlaku).
                                    Dilarang melakukan tindakan peretasan atau penyalahgunaan sistem.
                                </p>

                                <p><strong>5. Persetujuan</strong></p>
                                <!-- Mengubah nomor 6 menjadi 5 untuk konsistensi -->
                                <p style="text-align: justify">
                                    Dengan menggunakan situs ini, Anda menyetujui pengumpulan dan penggunaan data
                                    Anda sebagaimana dijelaskan dalam kebijakan ini.
                                </p>
                                <div class="space30"></div>

                                <p style="text-align: justify;">
                                    Jika Anda memiliki pertanyaan terkait kebijakan ini, silakan hubungi kami di:
                                </p>
                                <p style="text-align: center;">
                                    📧 Email: bprstaja@gmail.com <br>
                                    📞 Telp : (0354) 681755
                                </p>
                            </div>
                        </div>
                    </article>
                </div>
            </div>
        </div>
    </div>
</div>

<!--=====CTA AREA END=======-->
@endsection